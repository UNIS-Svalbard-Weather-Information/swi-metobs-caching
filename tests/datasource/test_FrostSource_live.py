        ...
        "data": [
            {
                ...
                "observations": [
                    {"elementId": "air_temperature", "value": -10.6, "unit": "degC"}
                ],
            },
            {
                "sourceId": "SN99840:0",
                ...
                "observations": [
                    {"elementId": "wind_speed", "value": 5.8, "unit": "m/s"},
                    {
        ...
        "data": [
            {
                ...
                "referenceTime": "2024-11-26T16:00:00.000Z",
                "observations": [
                    {"elementId": "air_temperature", "value": -10.5, "unit": "degC"},
                    {"elementId": "wind_speed", "value": 4, "unit": "m/s"},
                    {
                        "elementId": "wind_from_direction",
                        "value": 257,
                        "unit": "degrees",
    ...
    "data": [
        {
            ...
            "observations": [
                {"elementId": "air_temperature", "value": -12.1},
                {"elementId": "wind_speed", "value": 2.6},
                {"elementId": "wind_speed", "value": 2.6},
            ],
        },
        {
            ...
            "observations": [{"elementId": "wind_speed", "value": 3.2}],
        },
    ]
}
...
def test_fetch_station_data(frost_source, station):
    ...
    assert "data" in result, (
        f"Response missing 'data' field for station {station['id']}"
    )
    print(result)
...
def test_fetch_realtime_data(frost_source, station):
    ...
    assert result is not None, (
        f"Failed to fetch real-time data for station {station['id']}"
    )
    assert "timeseries" in result, (
        f"Response missing 'timeseries' for station {station['id']}"
    )
    assert len(result["timeseries"]) > 0, (
        f"No observations in real-time data for station {station['id']}"
    )
...
def test_fetch_timeseries_data(frost_source, station):
    ...
    assert result is not None, (
        f"Failed to fetch time series data for station {station['id']}"
    )
    assert "timeseries" in result, (
        f"Response missing 'timeseries' for station {station['id']}"
    )
    assert len(result["timeseries"]) > 0, (
        f"No observations in time series data for station {station['id']}"
    )
...
def test_transform_realtime_data(frost_source, station):
    ...
    assert transformed_data is not None, (
        f"Failed to transform real-time data for station {station['id']}"
    )
    assert "timeseries" in transformed_data, (
        f"Transformed data missing 'timeseries' for station {station['id']}"
    )
    assert len(transformed_data["timeseries"]) > 0, (
        f"No transformed observations for station {station['id']}"
    )
...
def test_transform_timeseries_data(frost_source, station):
    ...
    assert transformed_data is not None, (
        f"Failed to transform time series data for station {station['id']}"
    )
    assert "timeseries" in transformed_data, (
        f"Transformed data missing 'timeseries' for station {station['id']}"
    )
