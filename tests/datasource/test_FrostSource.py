...
def test_fetch_station_data(mock_get, frost_source):
    ...
    frost_source.logger.info.assert_called_with(
        f"Fetched station data for {station_id}"
    )
...
def test_fetch_realtime_data(mock_get, frost_source):
    ...
    frost_source.transform_realtime_data = MagicMock(
    ...
    mock_get.assert_called_once_with(expected_url, params=expected_params)
    frost_source.transform_realtime_data.assert_called_once_with(
        {"data": "realtime_data"}, station_id
    )
    frost_source.logger.info.assert_called_with(
        f"Fetched real-time data for {station_id}"
    )
...
def test_fetch_timeseries_data(mock_get, frost_source):
    ...
    frost_source.transform_timeseries_data = MagicMock(
        return_value={"transformed": "data"}
    )
    ...
    mock_get.assert_called_once_with(expected_url, params=expected_params)
    frost_source.transform_timeseries_data.assert_called_once_with(
        {"data": "timeseries_data"}, station_id, return_df=False
    )
    frost_source.logger.info.assert_called_with(
...
def test_transform_realtime_data(frost_source):
    ...
    raw_data = {
        "data": [
            {
                "referenceTime": "2023-01-01T01:00:00Z",
                "observations": [{"elementId": "air_temperature", "value": 5.0}],
            },
            {
                "referenceTime": "2023-01-01T00:30:00Z",
                "observations": [{"elementId": "humidity", "value": 80.0}],
            },
            {
                "referenceTime": "2022-12-31T23:00:00Z",
                "observations": [{"elementId": "wind_speed", "value": 10.0}],
            },
        ]
    }
    station_id = "SN18700"
...
def test_transform_realtime_data_no_data(frost_source):
    ...
    frost_source.logger.warning.assert_called_with(
        "No valid data found in real-time observations."
    )
    assert result is None
...
def test_is_station_online_recent_data(frost_source):
    ...
    station_id = "SN99857"
    recent_timestamp = (
        datetime.utcnow().replace(tzinfo=timezone.utc) - timedelta(minutes=30)
    ).isoformat()

    mock_data = {"timeseries": [{"timestamp": recent_timestamp}]}

    with patch.object(frost_source, "fetch_realtime_data", return_value=mock_data):
        assert (
            frost_source.is_station_online(station_id, max_inactive_minutes=120) is True
        )
...
def test_is_station_online_old_data(frost_source):
    ...
    station_id = "SN99857"
    old_timestamp = (
        datetime.utcnow().replace(tzinfo=timezone.utc) - timedelta(minutes=150)
    ).isoformat()

    mock_data = {"timeseries": [{"timestamp": old_timestamp}]}

    with patch.object(frost_source, "fetch_realtime_data", return_value=mock_data):
        assert (
            frost_source.is_station_online(station_id, max_inactive_minutes=120)
            is False
        )
        frost_source.logger.info.assert_called()
...
def test_is_station_online_invalid_timestamp(frost_source):
    ...
    """
    station_id = "SN99857"
    mock_data = {"timeseries": [{"timestamp": "invalid_timestamp"}]}
