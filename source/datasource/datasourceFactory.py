...
def get_datasource(station_id, config=None):
    ...
    if source_name not in DATASOURCE_MAPPING:
        ...
        if type == "mobile":
            logger.warning(
                f"Unknown datasource '{source_name}' for station_id {station_id}, defaulting to FrostBoatSource."
            )
            source_name = "FrostBoatSource"
        else:
            logger.warning(
                f"Unknown datasource '{source_name}' for station_id {station_id}, defaulting to FrostSource."
            )
